...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
        ...
        //表格导出_项目导出专用
        table.exportFile_project = function (id, data, type) {
            data = data || table.clearCacheKey(table.cache[id]);
            var vals = "<tr><td  colspan='25' style='font-weight: bold;font-size: 15px;border: 0px;' align='center'>项目进度汇总表</td></tr>"
            //表头
            vals += "<tr>";
            ...
            arry.forEach(function (item, index) {
                ...
                vals += "<td>" + item.department + "</td>";
                vals += "<td>" + item.manager + "</td>";
            ...
            template += '<body><table border="1">' + vals + '</table ></body></html >';
            //下载模板
...
//表格导出_项目导出专用
        table.exportFile_project1 = function (id, data, type) {
            data = data || table.clearCacheKey(table.cache[id]);
            type = type || 'csv';
            ...
            alink.href = 'data:' + textType + ';charset=utf-8,\ufeff' + encodeURIComponent(function () {
                var dataTop = [], dataTitle = [], dataMain = [];
                var top = "<tbody><tr><th colspan='25'>项目进度汇总表</th></tr>"
                dataTop.push(top);
                layui.each(data, function (i1, item1) {
                    var vals = [];
                    if (typeof id === 'object') { //ID直接为表头数据
                        ...
                        table.eachCols(id, function (i3, item3) {
                            if (item3.field && item3.type == 'normal' && !item3.hide) {
                                i1 == 0 && dataTitle.push(item3.title || '');
                                var item = item1[item3.field];
                                switch (item3.field) {
                                    case 'sflx':
                                        vals.push(item == 1 ? '是' : '否');
                                        break;
                                    case 'rjkfjd':
                                        switch (item) {
                                            case 0:
